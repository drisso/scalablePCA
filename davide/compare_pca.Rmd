...
```{r readin_dense, eval=FALSE}
tenx <- TENxMatrix("1M_neurons_data_dense_t.h5")
class(tenx)
is_sparse(tenx)
```

We use BiocSingular to run the actual PCA. We don't need `deferred=TRUE` here, since we are not using sparsity, but I'll keep it for consistency.

```{r pca_dense, eval=FALSE}
...
```{r biocsklearn_dense, eval=FALSE}
